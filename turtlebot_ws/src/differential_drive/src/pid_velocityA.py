...
######################################################
    ...
    #####################################################
    def __init__(self):
    ##################################################### this initialises all PID velocity controls and subscribers/publishers
        ...
        ### get parameters #### access parameters in the parameter server
        self.Kp = rospy.get_param('~Kp',800) #PID GAINS
        self.Ki = rospy.get_param('~Ki',10)
        self.Kd = rospy.get_param('~Kd',0.001)
        self.out_min = rospy.get_param('~out_min',-255) #PWM LIMITS
        self.out_max = rospy.get_param('~out_max',255)
        self.rate = rospy.get_param('~rate',30) #UPDATE RATES
        self.rolling_pts = rospy.get_param('~rolling_pts',2)
        self.timeout_ticks = rospy.get_param('~timeout_ticks',4)
        self.ticks_per_meter = rospy.get_param('ticks_meter', 20191)
        self.vel_threshold = rospy.get_param('~vel_threshold', 0.001) #WHATS THIS
        self.encoder_min = rospy.get_param('encoder_min', -2147483648) #ENCODER LIMITS LONG
        self.encoder_max = rospy.get_param('encoder_max', 2147483647)
        self.encoder_low_wrap = rospy.get_param('wheel_low_wrap', (self.encoder_max - self.encoder_min) * 0.3 + self.encoder_min ) #ESTIMATE POINT WHEN ENCODER OVERFLOWS
        ...
        rospy.Subscriber("Amotor_pub", Int16, self.wheelCallback) #Subscribe to Wheel encoder
        rospy.Subscriber("Awheel_vtarget", Float32, self.targetCallback) #Subscribe to twisttomotors vtargets
        self.pub_motor = rospy.Publisher('Amotor_cmd',Float32, queue_size=10) #publish motor command to motor. is this 1 motor or 2?
        self.pub_vel = rospy.Publisher('Awheel_vel', Float32, queue_size=10)  #publish wheel velocity to diff_tf
   
        
    #####################################################
    def spin(self):
    ##################################################### update current wheel encoders
    ...
    ##################################################### rolling average of velocity readings
